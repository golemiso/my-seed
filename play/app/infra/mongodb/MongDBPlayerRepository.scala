...
class MongoDBPlayerRepository(db: Future[DefaultDB])(implicit ec: ExecutionContext, mat: Materializer) extends PlayerRepository {
  ...
  override def resolve: Future[Seq[Player]] = {
    for {
      ...
      players <- collection.find(BSONDocument()).cursor[PlayerDocument]().documentSource().runWith(Sink.seq[PlayerDocument])
    } yield players
  }
  ...
  override def resolveBy(id: PlayerID): Future[Player] = {
    ...
    for {
      collection <- playersCollection
      player <- collection.find(query).cursor[PlayerDocument]().documentSource().runWith(Sink.head[PlayerDocument])
    } yield player
  }
